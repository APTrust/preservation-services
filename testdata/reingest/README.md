...
This folder, and the directories inside it, contain two versions of the same bag. The first version, in 01-Standard, uses the Standard storage option. The second, in 02-Glacier-OH, uses Glacier-Ohio.
...
Our test ingests the first (Standard) version of the bag, then reingests the other two. The test then verifies that after all three ingests are complete, the preserved files exist in standard storage only, and not in Glacier.

...
func main() {
	...
	worker := workers.NewObjectTransferValidator(
...
func printHelp() {
	message := `
object_transfer_validator validates an object
transfer by checking the metadata of each
transferred file and then performing a
fixity check on each transferred file.`
	fmt.Println(message)

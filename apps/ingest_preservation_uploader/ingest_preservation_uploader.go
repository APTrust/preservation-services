...
func main() {
	...
	worker := workers.NewIngestPreservationUploader(
...
func printHelp() {
	message := `
ingest_preservation_uploader copies files from the staging area to perservation
storage (and replication storage, when appropriate).`
	fmt.Println(message)

...
build: ## Build the Preservation-Services containers
	...
	@for app in $$(find ./apps -name *.go); do \
		...
		docker buildx build --platform=linux/arm64,linux/amd64 --build-arg PSERVICE=$$APP_NAME --build-arg OUTPUT_DIR=${OUTPUT_DIR} -t aptrust/$$APP_NAME:${DOCKER_TAG_NAME} -t aptrust/$$APP_NAME -f Dockerfile.build . ; \
	done

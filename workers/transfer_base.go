...
func NewTransferBase(context *common.Context, processorConstructor transfer.BaseConstructor, settings *Settings) *TransferBase {
	transferBase := &TransferBase{
		Base: Base{
			...
			transferProcessorConstructor: processorConstructor,
			institutionCache:             make(map[int64]string),
		},
	}

	// Handle SIGTERM & SIGINT
	signal.Notify(transferBase.KillChannel, syscall.SIGTERM, syscall.SIGINT)
	...
	transferBase.Base.ShouldSkipThis = transferBase.ShouldSkipThis
	transferBase.Base.GetTaskObject = transferBase.GetTaskObject
	...
	for i := 0; i < settings.NumberOfWorkers; i++ {
		...
		go transferBase.ProcessItem()
	}
	go transferBase.ProcessErrorChannel()
	go transferBase.ProcessFatalErrorChannel()
	go transferBase.ProcessSuccessChannel()

	return transferBase
}

func (b *TransferBase) ProcessSuccessChannel() {
...
func (b *TransferBase) ProcessErrorChannel() {
...
func (b *TransferBase) ProcessFatalErrorChannel() {
...
// WorkItem.
func (b *TransferBase) ShouldSkipThis(workItem *registry.WorkItem) bool {
	...
	/*
		if b.ObjectIsLocked(workItem) {
			b.Context.Logger.Warningf("Rejecting WorkItem %d because the target object is locked by...", workItem.ID)
			b.SaveWorkItem(workItem)
			return true
		}
	*/
...
func (b *TransferBase) FindOtherTransferRequests(workItem *registry.WorkItem) []*registry.WorkItem {
	v := url.Values{}
	v.Add("per_page", "20")
...
// receiving bucket.)
func (b *TransferBase) FindNewerTransferRequest(workItem *registry.WorkItem) *registry.WorkItem {
	items := b.FindOtherTransferRequests(workItem)
...
func (b *TransferBase) StillTransferringOlderVersion(workItem *registry.WorkItem) bool {
	items := b.FindOtherTransferRequests(workItem)
	for _, item := range items {
...
func (b *TransferBase) SupersededByNewerRequest(workItem *registry.WorkItem) bool {
	newerWorkItem := b.FindNewerTransferRequest(workItem)
	if newerWorkItem != nil && !b.IsLateStageOfTransfer() {
...
// current object.
func (b *TransferBase) IsLateStageOfTransfer() bool {
...
func (b *TransferBase) ShouldAbandonForNewerVersion(workItem *registry.WorkItem) bool {
	if !b.IsLateStageOfTransfer() {
...
func (b *IngestBase) ObjectAlreadyTransferred(workItem *registry.WorkItem) bool {

...
type TransferFile struct {
	...
	SourceStorageOption string           `json:"storage_option"`
	TargetStorageOption string           `json:"storage_option"`
	StorageRecords      []*StorageRecord `json:"storage_records"`
	UUID                string           `json:"uuid"`
}

func NewTransferFile(objIdentifier, pathInBag string) *TransferFile {
	return &TransferFile{
		...
		SourceStorageOption: "Standard",
		StorageRecords:      make([]*StorageRecord, 0),
	}
}

func TransferFileFromJSON(jsonData string) (*TransferFile, error) {
	f := &TransferFile{}
...
func (f *TransferFile) ToJSON() (string, error) {
...
func (f *TransferFile) Identifier() string {
...
func (f *TransferFile) IsParsableTagFile() bool {
...
func (f *TransferFile) SetChecksum(checksum *IngestChecksum) {
	updated := false
...
func (f *TransferFile) GetChecksum(source, algorithm string) *IngestChecksum {
...
func (f *TransferFile) SetStorageRecord(record *StorageRecord) {
	updated := false
...
func (f *TransferFile) GetStorageRecord(provider, bucket string) *StorageRecord {
...
func (f *TransferFile) IdentifierIsLegal() (bool, error) {
	var err error
...
func (f *TransferFile) ManifestChecksumRequired(manifestName string) bool {
	required := true
	fileType := f.FileType()
...
// match.
func (f *TransferFile) ChecksumsMatch(manifestName string) (bool, error) {
	ok := true
...
func (f *TransferFile) GetPutOptions() (minio.PutObjectOptions, error) {
...
func (f *TransferFile) HasPreservableName() bool {
	fileType := f.FileType()
...
// the provider + bucket.
func (f *TransferFile) NeedsSaveAt(provider, bucket string) bool {
...
func (f *TransferFile) HasRegistryURL(url string) bool {
...
func (f *TransferFile) GetTransferEvents() ([]*registry.PremisEvent, error) {
	if f.PremisEvents == nil {
		f.PremisEvents = make([]*registry.PremisEvent, 0)
	}
	var err error
	if len(f.PremisEvents) == 0 && f.NeedsSave {
		err = f.initTransferEvents()
	}
	return f.PremisEvents, err
}

func (f *TransferFile) initTransferEvents() error {
	transferEvent, err := f.NewFileTransferEvent()
	if err != nil {
		return err
	}
	...
	// In the case of a transfer, new identifier events are not needed
	// Because we are not changing the identifiers
	/*
	...
	*/
	...
	f.PremisEvents = append(f.PremisEvents, transferEvent)
...
func (f *TransferFile) URI() string {
	uri := ""
...
func (f *TransferFile) FindEvent(eventUUID string) *registry.PremisEvent {
...
func (f *TransferFile) ToGenericFile() (*registry.GenericFile, error) {
	transferEvents, err := f.GetTransferEvents()
	if err != nil {
		return nil, err
	}
	...
	// change the existing genericfile
	// id, source, dest, didCopyWork
	// change storageurls
	return &registry.GenericFile{
		...
		PremisEvents:         transferEvents,
		Size:                 f.Size,
		State:                constants.StateActive,
		StorageOption:        f.TargetStorageOption, // change this - save as source or target option
...
// NewFileTransferEvent returns a PremisEvent describing a file transfer.
// Param storedAt should come from the TransferFile's primary StorageRecord.
// Param md5Digest should come from the TransferFiles md5 Checksum record.
// Param _uuid should come from TransferFile.UUID.
func (f *TransferFile) NewFileTransferEvent() (*registry.PremisEvent, error) {
	...
	return &registry.PremisEvent{
		...
		Detail:                       fmt.Sprintf("Completed transfer to preservation storage (%s)", f.UUID),
...
func (f *TransferFile) NewFileFixityCheckEvent(manifestChecksum *IngestChecksum) *registry.PremisEvent {
	eventId := uuid.New()
	timestamp := time.Now().UTC()
...
func (f *TransferFile) NewFileDigestEvent(ingestChecksum *IngestChecksum) *registry.PremisEvent {
	eventId := uuid.New()
	timestamp := time.Now().UTC()
...
func (f *TransferFile) NewFileIdentifierEvent(identifier, identifierType string) (*registry.PremisEvent, error) {
	if identifier == "" {
...
func (f *TransferFile) NewFileReplicationEvent(replicationRecord *StorageRecord) (*registry.PremisEvent, error) {
...
func transferGetFixityProps(fixityAlg string, fixityMatched bool) map[string]string {

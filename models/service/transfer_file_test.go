...
const eTransferObjIdent = "test.edu/obj"
const eTransferPathInBag = "data/file.txt"
const eTransferFileIdent = "test.edu/obj/data/file.txt"

func TestNewTransferFile(t *testing.T) {
	...
	assert.Equal(t, "Standard", f.SourceStorageOption)
	assert.Equal(t, "Glacier", f.TargetStorageOption)
	assert.NotNil(t, f.StorageRecords)
}

func TestTransferFileFromJson(t *testing.T) {
	expectedFile := testutil.GetTransferFile(true, true)
	f, err := service.TransferFileFromJSON(TransferFileJson)
	assert.Nil(t, err)
...
func TestTransferFileToJson(t *testing.T) {
	f := testutil.GetTransferFile(true, true)
	data, err := f.ToJSON()
	assert.Nil(t, err)
	assert.Equal(t, TransferFileJson, data)
}

func TestTransferIdentifier(t *testing.T) {
	f := service.NewTransferFile(testutil.ObjIdentifier, "data/image.jpg")
	assert.Equal(t, "test.edu/test-bag/data/image.jpg", f.Identifier())
}

func TestTransferInstitution(t *testing.T) {
	f := service.NewTransferFile(testutil.ObjIdentifier, "data/image.jpg")
	assert.Equal(t, "test.edu", f.Institution())
}

func TestTransferFileType(t *testing.T) {
...
func TestTransferIsParsableTagFile(t *testing.T) {
	f := testutil.GetTransferFile(false, false)
...
func TestTransferSetChecksum(t *testing.T) {
	f := testutil.GetTransferFile(false, false)
...
func TestTransferToGenericFile(t *testing.T) {
	f := getFileForEvents()
	f.NeedsSave = true
...
func TestTransferToGenericFile_StorageRecords(t *testing.T) {
	f := getFileForEvents()
	f.NeedsSave = true
	gf, err := f.ToGenericFile()
	require.Nil(t, err)
...
func TestTransferHasPreservableName(t *testing.T) {
	no := []string{
		"bagit.txt",
		"fetch.txt",
		"manifest-sha256.txt",
...
func TestTransferFileFindEvent(t *testing.T) {
...
func getTransferFileForEvents() *service.IngestFile {
...
func TestNewFileTransferEvent(t *testing.T) {
	f := getFileForEvents()
...
func TestNewTransferFileFixityCheckEvent(t *testing.T) {
	f := getFileForEvents()
...
func TestNewTransferFileDigestEvent(t *testing.T) {
	f := getFileForEvents()
...
func TestNewTransferFileIdentifierEvent(t *testing.T) {
	f := getFileForEvents()
...
func TestNewTransferFileReplicationEvent(t *testing.T) {
	f := getFileForEvents()
...
const TransferFileJson = `{"checksums":[{"algorithm":"md5","datetime":"0001-01-01T00:00:00Z","digest":"md5:ingest","source":"ingest"},{"algorithm":"md5","datetime":"0001-01-01T00:00:00Z","digest":"md5:registry","source":"registry"}],"copied_to_staging_at":"0001-01-01T00:00:00Z","error_message":"no error","file_format":"text/javascript","file_modified":"1904-06-16T15:04:05Z","format_identified_at":"0001-01-01T00:00:00Z","id":999,"institution_id":9855,"intellectual_object_id":4432,"is_reingest":false,"mtime":"0001-01-01T00:00:00Z","needs_save":true,"object_identifier":"test.edu/some-bag","path_in_bag":"data/text/file.txt","registry_urls":[],"saved_to_registry_at":"0001-01-01T00:00:00Z","size":5555,"storage_option":"Standard","storage_records":[{"bucket":"","error":"","etag":"","provider":"","size":0,"stored_at":"1904-06-16T15:04:05Z","url":"https://example.com/storage/record/1","verified_at":"0001-01-01T00:00:00Z"},{"bucket":"","error":"","etag":"","provider":"","size":0,"stored_at":"1904-06-16T15:04:05Z","url":"https://example.com/storage/record/2","verified_at":"0001-01-01T00:00:00Z"}],"uuid":"00000000-0000-0000-0000-000000000000"}`
